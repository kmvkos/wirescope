...
def test_root_loads_versioned_pcap_management_script(api_context):
    ...
    assert "/static/pcap_management.js?v=20260827-ui31" in response.text
...
def test_pcap_management_ui_hides_manually_deleted_rows_but_preserves_history():
    ...
    assert "function syncDeleteButton(row, session)" in script
    assert "button.dataset.captureJobId !== jobId" in script
