...
The default `pytest` invocation excludes `@pytest.mark.network`. Passive and
active end-to-end tests use fixtures and do not scan the live network. Live
Nmap tests require an explicit `WIRESCOPE_LIVE_SCOPE` and
`pytest -m network`.
